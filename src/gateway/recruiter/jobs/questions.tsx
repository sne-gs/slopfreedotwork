...
import { editQuestionsEndpoint } from "./endpoints";
import {
...
questions.post("/", async (c: Context<AppEnv>) => {
	...
	if (form.has("questionAction") && !action) {
		return statusResponse("Unknown question action");
	}
	...
	const saved = { ...parsed, questions: list };
	if (job.status === "draft") {
		await mgr.saveDraft(jobId, saved);
	} else {
		await mgr.saveJobContent(jobId, saved);
	}

	const questionsEndpoint = editQuestionsEndpoint(jobId);

	const fragment = await renderFragment(
		<QuestionEditor questions={list} questionsEndpoint={questionsEndpoint} />,
	);
